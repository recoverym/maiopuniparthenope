...
function refreshScores(checkbox, scoring) {
  ...
  if (checkbox.checked) {
	  jQuery(prefix + 'score-if-chosen').val('1');
	  jQuery(prefix + 'score-if-not-chosen').val('0');
  }
  else {
    if (scoring == 0) {
    	jQuery(prefix + 'score-if-not-chosen').val('0');
      if (jQuery('#edit-choice-multi').attr('checked')) {
    	  jQuery(prefix + 'score-if-chosen').val('-1');
      }
      else {
    	  jQuery(prefix + 'score-if-chosen').val('0');
      }
    }
    else if (scoring == 1) {
    	jQuery(prefix + 'score-if-chosen').val('0');
	    if (jQuery('#edit-choice-multi').attr('checked')) {
	    	jQuery(prefix + 'score-if-not-chosen').val('1');
	    }
	    else {
	    	jQuery(prefix + 'score-if-not-chosen').val('0');
	    }
...
function refreshCorrect(textfield) {
  ...
  if (isChosen(textfield.id)) {
    ...
    notChosenScore = new Number(jQuery(prefix + 'score-if-not-chosen').val());
  }
  else {
    chosenScore = new Number(jQuery(prefix + 'score-if-chosen').val());
  ...
  if(notChosenScore < chosenScore) {
	  jQuery(prefix + 'correct').attr('checked', true);
  }
  else {
	  jQuery(prefix + 'correct').attr('checked', false);
  }
}
